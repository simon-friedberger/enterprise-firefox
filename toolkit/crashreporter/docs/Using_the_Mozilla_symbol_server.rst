...
We have jobs listed in ``./mach try`` that will automatically upload the debug
symbols for your try builds to the symbol server. For example, if you select
``build-win64-shippable/opt-upload-symbols`` it will run after
``build-win64-shippable/opt`` and upload the corresponding debug symbols once
the build is done. Search for ``-upload-symbols`` to find all compatible jobs.
