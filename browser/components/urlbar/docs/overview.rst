...
.. code:: JavaScript
  ...
  UrlbarQueryContext {
    ...
    muxer; // {string} Name of a registered muxer. Muxers can be registered
           // through the ProvidersManager.
    providers; // {array} List of registered provider names. Providers can be
               // registered through the ProvidersManager.
...
The *ProvidersManager* is a singleton, it registers internal providers on
...
The *Controller* starts and stops queries through the *ProvidersManager*.
...
registered as *Objects* through the *ProvidersManager*.
